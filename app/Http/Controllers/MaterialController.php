...
class MaterialController extends Controller
{
  ...
  public function getData()
  {
    ...
    ->addColumn('quiz_category', function($row){
      return $row->category_name; // Change this line
    })
  ...
  public function index()
  {
    ...
    if (Auth::user()->hasRole('admin')) {
      ...
      $material = Material::whereIn('created_by',$admin_id)->get()->sortBy('name');
    } else {
      $school_id = Auth::user()->school_id;
      $teacher = User::where('school_id',$school_id)->whereHas('lecture')->get();
      $teacher_id = [];
      ...
      $material = Material::whereIn('created_by',$teacher_id)->get()->sortBy('name');
    }
    return view('material.index', compact('quiztype','quizcategory','material'));
  }
  ...
  public function store(Request $request)
  {
    ...
    $rules = [
      'quiz_type' => 'required',
      'name' => 'required|max:150|unique:materials',
  ...
  public function api_index($id){
    ...
    // merged id
    $merged_id = array_merge($teacher_id, $admin_id);
  ...
  public function api_show($idType, $idMaterial) {
    $data = Material::with('modules')->find($idMaterial);
  
    return responseAPI(200, true, $data);
  }

  public function api_module_detail($idType, $idMaterial, $idModule) {
    $data = MaterialModule::find($idModule);
